...
## v1.15.1 - 用户体验改进与Windows打包 (2025-12-16)

### 🎨 UI/UX改进

#### 1. 视频缩略图显示 🖼️
- 上传视频后自动显示缩略图预览
- 使用OpenCV (cv2)提取视频第一帧
- 自动缩放适应显示区域
- 降级处理：无OpenCV时显示文件信息
...
- 新增 `generate_video_thumbnail()` 方法（使用OpenCV）
- 改进 `select_video()` 方法支持工程集成
- 添加 `resizeEvent()` 响应式缩放
- 添加 `dragMoveEvent()` 修复拖拽问题
- 改进拖拽事件处理逻辑
- OpenCV集成（可选依赖）
...
- 减少启动时间约70ms
- 改进OpenCV错误处理，静默失败
- 使用OpenCV替代ffmpeg生成缩略图

### 📦 Windows打包

- 新增完整的Windows打包脚本
- 支持BAT和PowerShell两种打包方式
- 使用国内镜像源（清华）加速依赖下载
- 解决中文乱码问题（UTF-8编码）
- 新增PyInstaller配置文件
- 新增Windows版本信息文件
...
## v1.15.0 - 参考生视频功能 (2025-12-16)

### 🎯 核心更新

新增参考生视频（Reference Video to Video）功能，支持基于参考视频生成新视频。
...
#### API客户端扩展
- `upload_video_and_get_url`：视频上传方法
...
- 创建`REFERENCE_VIDEO_TO_VIDEO_GUIDE.md`完整功能说明
- 包含API调用示例
- 提供Python上传代码示例
- 详细的参数说明和使用流程
